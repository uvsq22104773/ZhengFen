...
public class SequenceQuartetsCombination extends AbstractCombinaison {

    Integer rank;

    @Override
    public boolean estValide(List<Carte> cartes) {
        if (cartes.isEmpty() || cartes.size() % 4 != 0 || cartes.size() < 12) {
            return false;
        }
        ...
        rank = 100 * cartes.size() / 4 + list.get(list.size() - 1);
        return true;
    }

    @Override
    public String getNom() {
        return "SequenceOfQuartets";
    }
